#!/usr/bin/env python3
"""Run a JSON STT push/fold scenario through the candidate-analysis pipeline."""
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Run a JSON STT push/fold scenario through the pipeline."
    )
    parser.add_argument("scenario", help="path to a JSON STT push/fold scenario")
    parser.add_argument(
        "--horizon",
        type=int,
        default=None,
        help="repeated-game horizon (default: scenario repeated.horizon)",
    )
    parser.add_argument(
        "--discount",
        type=float,
...
def main(argv) -> int:
    ...
    result = run_stt_pushfold_analysis(args.scenario, config)

    counts = result.pipeline_result.filter_result.summary_counts
    print(f"scenario_id: {result.scenario_id}")
    print(f"format_version: {result.scenario.format_version}")
    print("value_unit: modelled tournament prize EV delta")
