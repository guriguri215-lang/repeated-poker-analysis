#!/usr/bin/env python3
"""Serve a minimal local GUI for editing an equity-matrix scenario (prototype v1).

Usage:

    python scripts/serve_equity_matrix_gui.py --port 8003
    # then open http://127.0.0.1:8003/ in a browser

This is a pre-GUI-product prototype: a local-only web page that loads an
equity-matrix scenario JSON into a form (top-level fields, a table of weighted
Hero buckets, a table of weighted Villain buckets, and a Hero x Villain equity
matrix whose cells are the Hero pot share before rake, a number in [0, 1]), lets
you add / remove / edit buckets, rebuild the matrix, validate the form, and save it
back to JSON. It is the equity flavour of ``scripts/serve_showdown_matrix_gui.py``
and uses the standard library only (``http.server`` plus inline HTML / CSS /
vanilla JavaScript); it adds no framework or dependency, and no new solver, model,
or analysis logic.

The equity values are abstract Hero pot shares supplied in the JSON (1.0 = Hero
wins, 0.5 = chop, 0.0 = Villain wins); they are not computed from real cards.

Scope (v1): equity-matrix mode, editing (load / edit Hero buckets / edit Villain
buckets / edit matrix cells / validate / save), and running the candidate analysis
for the current matrix form. Single-hand, Hero-range-only, showdown-matrix, and
...
* ``GET  /``            -- the equity-matrix editor page;
* ``POST /api/load``    -- ``{"path": "..."}`` -> the form fields of that
  equity-matrix scenario (a non-equity-matrix scenario is rejected);
...
def _buckets_from_payload(raw_list, field_name, convert) -> list:
    ...
    :func:`validate_equity_matrix_form` reports it as a ``<field>[i]`` message
...
def _equity_cell_value(raw):
    """Convert one equity cell to a float when possible, else keep it as-is.
    ...
    is kept unchanged so :func:`validate_equity_matrix_form` reports it as a bad
    cell rather than the conversion raising or the value being silently coerced
    (in particular it is never rounded to a default like ``0.5``).
    """
...
def _equity_matrix_from_payload(raw):
    """Convert the nested equity matrix, parsing numeric cells to ``float``.

    A non-object matrix or a non-object row is kept as-is so the validator reports
    it instead of the conversion raising. Each cell is converted with
    :func:`_equity_cell_value` (numeric strings -> float; anything else kept).
    """
...
def _form_from_payload(payload) -> EquityMatrixScenarioForm:
    """Build an EquityMatrixScenarioForm from the browser's flat dict.
    ...
    The ``equity_matrix`` numeric cells are parsed to floats where possible (a
    non-numeric cell is kept for the validator); a non-object matrix is also kept
    as-is. A non-list bucket section or a bad numeric bucket value raises
    :class:`ValueError`.
    """
    ...
    if "equity_matrix" in payload:
        form.equity_matrix = _equity_matrix_from_payload(payload["equity_matrix"])
    return form


def _form_to_payload(form: EquityMatrixScenarioForm) -> dict:
...
def api_load(payload) -> dict:
    """Load an equity-matrix scenario file into form fields for the browser."""
    ...
    if mode != "equity-matrix":
        raise ValueError(
            "equity-matrix GUI supports the equity_matrix mode; "
            f"this scenario is {mode} mode"
        )
    form = equity_matrix_form_from_dict(data)
...
def api_save(payload) -> dict:
    ...
    try:
        out_dict = equity_matrix_form_to_dict(form)
...
def api_analyze(payload) -> dict:
    """Run the candidate analysis for the equity-matrix form the browser sent.
...
<head>
<meta charset="utf-8">
<title>Equity-matrix scenario editor (prototype)</title>
<style>
...
<body>
<h1>Equity-matrix scenario editor (local prototype)</h1>
<p class="hint">Local-only equity_matrix editor. Load a scenario JSON, edit the
weighted Hero and Villain buckets and the Hero x Villain equity matrix (each cell
is the Hero pot share before rake, a number in [0, 1]: 1.0 = Hero wins, 0.5 = chop,
0.0 = Villain wins), validate, save, and run the candidate analysis for the current
form values. These equities are abstract values from the JSON, not computed from
real cards. Single-hand / Hero-range-only / showdown-matrix / betting-tree scenarios
...
<fieldset>
  ...
  <div class="row">
    <label><span>scenario path</span>
      <input type="text" id="load_path" placeholder="examples/scenarios/range_equity_steal_bet98.json"></label>
...
<fieldset>
  ...
  come from the equity matrix below.</p>
  <table>
    <thead>
      <tr>
...
<fieldset>
  <legend>Equity matrix</legend>
  <p class="hint">Each cell is the Hero pot share before rake for that Hero (row) vs
  Villain (column) pairing: a number in [0, 1]. After editing bucket hand_ids, click
  "Rebuild matrix" to regenerate the grid; cells for matching ids are kept and new
  cells default to 0.5. Out-of-range or non-numeric cells are reported by Validate.</p>
...
<fieldset>
  ...
  <p class="hint">Runs the candidate analysis for the current equity-matrix form
  values (no file needed). Equity-matrix only; shows candidate counts and the
...
// Snapshot the current matrix inputs into a nested object keyed by the hero/
// villain ids they were built for, so a rebuild can keep matching cells.
function snapshotMatrix() {
  var snap = {};
  document.querySelectorAll("#matrix_body input[data-hero]").forEach(function (inp) {
    var h = inp.getAttribute("data-hero");
    var v = inp.getAttribute("data-villain");
    if (!snap[h]) { snap[h] = {}; }
    snap[h][v] = inp.value;
  });
  return snap;
...
function makeCell(heroId, villainId, value) {
  ...
  inp.type = "text";
  inp.className = "matrix_cell";
  inp.setAttribute("data-hero", heroId);
  inp.setAttribute("data-villain", villainId);
...
// Seed the matrix inputs from a loaded matrix object before rebuilding so that
...
function collectForm() {
  ...
  form.equity_matrix = collectMatrix();
  return form;
}
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Serve a minimal local GUI for editing an equity-matrix scenario."
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
...
def main(argv) -> int:
    ...
    print(f"serving equity-matrix scenario GUI at http://{host}:{port}/ (Ctrl+C to stop)")
    try:
