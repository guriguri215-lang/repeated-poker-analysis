...
Generated results, commercial solver exports, and private hand histories are not committed. Every exported analysis includes a run manifest block recording the scenario-file SHA-256, the scenario format version, the package version, the best-effort git commit of the code, a UTC timestamp, and the effective analysis parameters (horizon, discount, response mode, tolerances). The plan originally called for a separate `run_manifest.json` file with a dependency lock and a seed; the implementation embeds the manifest in the JSON / CSV / Markdown exports instead, and a dependency lock and a seed are not applicable to the standard-library-only, deterministic analysis.
