...
def test_analyze_invalid_equity_cell_does_not_analyze(bad_cell):
    # An out-of-range or non-numeric equity cell is reported (and not rounded); the
    # analysis is not run.
    form = _loaded_form()
    form["equity_matrix"]["hero_medium"]["villain_weak"] = bad_cell
    result = gui.api_analyze({"form": form})
    assert result["ok"] is False
    assert result["valid"] is False
    assert any(
...
def test_analyze_bad_bucket_value_is_error():
    form = _loaded_form()
